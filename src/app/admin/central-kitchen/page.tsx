...
export default function CentralKitchenPage() {
  ...
  const handleConfirmReset = async () => {
    setResetting(true);
    try {
      const res = await fetch(`/api/central-kitchen/daily?date=${date}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Lỗi khi đặt lại số liệu");
      toast.success(data.message || `Đã đặt lại toàn bộ số liệu ngày ${date}`);
      setShowResetConfirm(false);
      await fetchData();
    } catch (err: any) {
      toast.error(err.message || "Không thể đặt lại số liệu ngày");
    } finally {
      setResetting(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2 self-start md:self-auto">
          ...
          {isSettingsAllowed && (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 bg-red-50 hover:bg-red-100 dark:bg-red-950/40 dark:hover:bg-red-900/50 text-red-600 dark:text-red-400 rounded-xl text-xs font-bold border border-red-200 dark:border-red-800/60 transition active:scale-95 cursor-pointer shadow-xs"
              title={`Đặt lại toàn bộ số liệu ngày ${date}`}
            >
              <RotateCcw className={`w-3.5 h-3.5 ${resetting ? "animate-spin" : ""}`} />
              <span className="hidden sm:inline">Đặt lại ngày</span>
      ...
      {/* MODAL CẢNH BÁO XÁC NHẬN ĐẶT LẠI SỐ LIỆU NGÀY */}
      {showResetConfirm && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4 backdrop-blur-xs">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full p-6 shadow-2xl border border-red-200 dark:border-red-900/60 text-left relative animate-in fade-in zoom-in-95 space-y-4">
            <div className="flex items-start gap-3.5">
              <div className="w-12 h-12 rounded-2xl bg-red-100 dark:bg-red-950 text-red-600 dark:text-red-400 flex items-center justify-center shrink-0 border border-red-200 dark:border-red-800">
                <AlertTriangle className="w-6 h-6" />
              </div>
              <div className="flex-1">
                <h4 className="text-base sm:text-lg font-black text-slate-900 dark:text-white">
                  Xác nhận đặt lại số liệu ngày?
                </h4>
            ...
            <div className="p-4 rounded-2xl bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900/60 text-xs text-red-900 dark:text-red-300 space-y-2 leading-relaxed">
              <div className="font-extrabold flex items-center gap-1.5 text-red-700 dark:text-red-300 uppercase tracking-wide text-[11px]">
                <span>⚠️</span> Cảnh báo hành động không thể hoàn tác:
              </div>
              <ul className="list-disc pl-4 space-y-1.5 text-[11px] sm:text-xs text-red-800 dark:text-red-300/90">
                <li>
                  Toàn bộ số suất ăn (mặn, chay, cháo) của <strong>tất cả 4 chi nhánh</strong> trong ngày <strong>{date}</strong> sẽ bị xóa về 0.
                </li>
                <li>
                  Trạng thái chốt đi chợ và chốt ăn sẽ quay về ban đầu (<strong>⏳ Chưa chốt</strong>).
                </li>
                <li>
                  Màn hình Tivi xưởng bếp và các bảng thống kê sẽ cập nhật về <strong>0 suất</strong> ngay lập tức.
                </li>
              </ul>
            </div>
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2">
              ...
              <button
                ...
                disabled={resetting}
                className="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl text-xs sm:text-sm font-bold shadow-md shadow-red-600/30 transition disabled:opacity-50 cursor-pointer"
              >
