  ...
  useEffect(() => {
    ...
    branches.forEach((b) => {
      initial[b.branchId] = {
        ...
        noodleId: b.noodleId || (ingredients.noodles[0]?.id ?? ""),
        noodleName: b.noodleName || (ingredients.noodles[0]?.name ?? "Bánh phở"),
        fruitId: b.fruitId || (ingredients.fruits[0]?.id ?? ""),
        fruitName: b.fruitName || (ingredients.fruits[0]?.name ?? "Chuối"),
        note: b.note || "",
                ...
                <div className="bg-slate-50 dark:bg-slate-800/60 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <button
                      type="button"
                      disabled={isLocked}
                      onClick={() => updateField(branch.branchId, "manMealType", "NUOC")}
                      className={`flex items-center justify-center gap-2 py-2 px-3 rounded-xl font-bold text-xs sm:text-sm transition ${
                  ...
                  {val.manMealType === "NUOC" && (
                    <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-700">
                      ...
                      <select
                        ...
                        disabled={isLocked}
                        onChange={(e) =>
                          updateField(branch.branchId, "noodleId", e.target.value)
                        }
                        className="w-full text-xs sm:text-sm font-semibold p-2 rounded-lg border border-blue-300 dark:border-blue-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:outline-none"
                      >
